...
1. Add environment variable `TASK_TRACKER_CONNECTIONSTRINGS__TASKSDB` to define the connection string to the tasks db.
...
2. Restore db schema and seed `task_item` table with test data:
    ...
    > [!NOTE]
    > Ignore following db migrations error. This is known behavior of the Npgsql EF Core provider. Rather than check existence of `__EFMigrationsHistory` table, it runs `SELECT` query against it and handles exception if the table cannot be found.
    >
